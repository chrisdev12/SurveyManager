...
{
    public class GeneralSurveyControllerTest : IntegrationTestBase
    {
        private readonly string indexHealthCheckEndpoint = "/v1/generalSurvey";

        [Fact]
        public async Task TestGeneralSurveyController_shouldAcceptRequest()
        {
            var response = await _client.PostAsync(indexHealthCheckEndpoint, ComposeBodyOnQueryString());

            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        }

        private FormUrlEncodedContent ComposeBodyOnQueryString()
        {
